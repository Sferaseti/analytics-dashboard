...
export async function getStripePrices() {
  // Return mock data if Stripe key is not properly configured
  ...
  try {
    ...
    console.warn('Failed to fetch Stripe prices, using mock data:', error);
    return [
      {
        id: 'price_mock_base',
        productId: 'prod_mock_base',
...
export async function getStripeProducts() {
  ...
  if (!process.env.STRIPE_SECRET_KEY || process.env.STRIPE_SECRET_KEY.includes('placeholder') || process.env.STRIPE_SECRET_KEY.length < 50) {
    return [
      ...
      },
      {
        id: 'prod_mock_plus',
        name: 'Plus',
        description: 'Plus plan with advanced features',
  ...
  try {
    ...
    console.warn('Failed to fetch Stripe products, using mock data:', error);
    return [
      {
        id: 'prod_mock_base',
        name: 'Base',
